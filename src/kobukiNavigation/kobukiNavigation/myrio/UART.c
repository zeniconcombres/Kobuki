...
#define VisaDefaultTimeout 10000
                  ...
                  const Uart_Parity parity)
{
    ...
    status = viSetAttribute(port->session, VI_ATTR_ASRL_END_IN, VI_ASRL_END_NONE);
 ...
 * @return      int32_t   Error/success status
 */
int32_t Uart_Flush(MyRio_Uart* port)
{
    int32_t status = VI_SUCCESS;
    int32_t oldTimeout = 0;
    uint8_t read[0xFF] = {0};
   ...
   status= viFlush(port->session,VI_IO_IN_BUF);
